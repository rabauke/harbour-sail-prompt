      ...
      m_settings("harbour-sail-prompt", "harbour-sail-prompt")
{
    ...
    connect(m_api, SIGNAL(getModelsFinished(QList<open_ai_api::Model>,open_ai_api::Error)),
            this, SLOT(onGetModelsFinished(QList<open_ai_api::Model>,open_ai_api::Error)));
    connect(m_api, SIGNAL(streamingChatReply(QString)),
            this, SLOT(onStreamingChatReply(QString)));
    connect(m_api, SIGNAL(streamingChatFinished(QString,open_ai_api::Error)),
            this, SLOT(onStreamingChatFinished(QString,open_ai_api::Error)));
...
QString AppModel::baseUrl() const { return m_baseUrl; }
...
QString AppModel::apiKey() const { return m_apiKey; }
...
QString AppModel::model() const { return m_model; }
...
QStringList AppModel::models() const { return m_models; }

QString AppModel::systemPrompt() const { return m_systemPrompt; }
...
ChatMessageListModel* AppModel::messages() const { return m_messages; }
bool AppModel::busy() const { return m_busy; }
bool AppModel::configured() const {
    const QUrl url(m_baseUrl);
    return url.isValid() && !url.scheme().isEmpty() && !url.host().isEmpty() && !m_apiKey.trimmed().isEmpty();
}
bool AppModel::modelsLoading() const { return m_modelsLoading; }
bool AppModel::selectedModelAvailable() const { return !m_model.isEmpty() && m_models.contains(m_model); }
bool AppModel::canSend() const { return configured() && selectedModelAvailable() && !m_busy && !m_modelsLoading; }
bool AppModel::canClear() const { return !m_busy; }

bool AppModel::exporting() const { return m_exporting; }
QString AppModel::exportError() const { return m_exportError; }
bool AppModel::exportSuccess() const { return m_exportSuccess; }
QString AppModel::exportPath() const { return m_exportPath; }
QString AppModel::exportMessage() const { return m_exportMessage; }

QString AppModel::renderedDocument() const {
    return m_renderedDocument;
}

bool AppModel::settingsGuidanceShown() const { return m_settingsGuidanceShown; }

SessionListModel* AppModel::history() const { return m_sessionListModel; }

void AppModel::saveCurrentSession() {
    if (m_busy) return;
    if (m_messages->rowCount() == 0) return;

    Session session;
    session.id = m_currentSessionId;
    for (int i = 0; i < m_messages->rowCount(); ++i) {
        ChatMessage* msg = m_messages->get(i);
        if (msg->role() != ChatMessage::User && msg->role() != ChatMessage::Agent) continue;
...
void AppModel::addUserMessage(const QString& content) {
    ...
    for (int i = 0; i < m_messages->rowCount(); ++i) {
        ...
        switch (msg->role()) {
            case ChatMessage::System: apiMsg.role = "system"; break;
            case ChatMessage::User: apiMsg.role = "user"; break;
            case ChatMessage::Agent: apiMsg.role = "assistant"; break;
        }
...
void AppModel::applyConfig(const QString& url, const QString& apiKey, const QString& systemPrompt) {
    setBaseUrl(url);
    setApiKey(apiKey);
...
void AppModel::onGetModelsFinished(const QList<open_ai_api::Model>& models, const open_ai_api::Error& error) {
    m_modelsLoading = false;
    emit modelsLoadingChanged();
    emit canSendChanged();
...
void AppModel::onStreamingUpdateTimeout() {
    if (m_pendingStreamingContent.isEmpty()) return;

    if (m_assistantMessageIndex != -1) {
        ChatMessage* msg = m_messages->get(m_assistantMessageIndex);
        if (msg)
            m_messages->setContent(m_assistantMessageIndex, msg->content() + m_pendingStreamingContent);
    }
    m_pendingStreamingContent.clear();
    updateRenderedDocument();
}
