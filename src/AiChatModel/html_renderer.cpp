...
QString HtmlRenderer::render(const QList<ChatMessage*>& messages) {
    QString html = "<html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><style>"
                   "body { font-family: sans-serif; color: #E6E6E6; background-color: transparent; margin: 0; padding: 10px; line-height: 1.4; }"
                   ".message { margin-bottom: 24px; padding: 12px; border-radius: 12px; word-wrap: break-word; }"
                   ".user { background-color: rgba(255, 255, 255, 0.15); margin-left: 20px; border-bottom-right-radius: 4px; }"
                   ".agent { background-color: rgba(255, 255, 255, 0.08); margin-right: 20px; border-bottom-left-radius: 4px; }"
                   "pre { background-color: rgba(0, 0, 0, 0.4); padding: 12px; border-radius: 6px; overflow-x: auto; white-space: pre-wrap; font-size: 0.9em; }"
                   "code { font-family: monospace; background-color: rgba(0, 0, 0, 0.3); padding: 2px 4px; border-radius: 3px; }"
    ...
    for (int i = 0; i < messages.size(); ++i) {
        ...
        if (msg->role() == ChatMessage::System) continue;
    ...
    for (int i = 0; i < lines.size(); ++i) {
        ...
        if (line.startsWith("#")) {
            ...
            if (level > 0 && level <= 6 && level < line.length() && line[level] == ' ') {
                ...
                flushParagraph();
                closeList();
                html += QString("<h%1>").arg(level) + processInline(content) + QString("</h%1>").arg(level);
                continue;
            }
        }
