...
QVariant SessionListModel::data(const QModelIndex &index, int role) const {
    ...
    switch (role) {
        ...
        case TimestampRole: return session.timestamp;
        case PreviewRole:
...
void SessionListModel::deleteSessionById(const QString& id) {
    ...
    for (int i = 0; i < m_sessions.size(); ++i) {
        if (m_sessions.at(i).id == id) { index = i; break; }
    }
    if (index < 0) return;
...
QList<ChatMessage*> SessionListModel::messagesById(const QString& id) const {
    for (int i = 0; i < m_sessions.size(); ++i) {
        if (m_sessions.at(i).id == id) return messages(i);
    }
