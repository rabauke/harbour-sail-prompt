...
SessionStore::SessionStore(QObject *parent) : QObject(parent) {
    m_storagePath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/sessions";
    QDir dir;
...
QString SessionStore::storagePath() const { return m_storagePath; }
QString SessionStore::lastError() const { return m_lastError; }
...
bool SessionStore::isValidId(const QString& id) {
    static const QRegExp uuidPattern("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$");
...
QList<Session> SessionStore::loadAllSessions() {
    ...
    for (int fileIndex = 0; fileIndex < files.size(); ++fileIndex) {
        ...
        for (int i = 0; i < msgArray.size(); ++i) {
            ...
            if (role == ChatMessage::User) hasUserMessage = true;
        }
    ...
    if (session.title.isEmpty()) {
        for (int i = 0; i < session.messages.size(); ++i) {
            ...
            if (msg->role() == ChatMessage::User) {
                ...
                if (normalized.length() > 50) session.title += "...";
                break;
            }
        }
    }
