...
int main(int argc, char *argv[])
{
    ...
    if (!require(temporaryDirectory.isValid(), "temporary directory creation")) return 1;
    const QDateTime timestamp(QDate(2026, 7, 26), QTime(22, 23, 0));
    const QString firstPath = PdfExporter::outputPath(temporaryDirectory.path(), "A title", timestamp);
    QFile collision(firstPath);
    if (!require(collision.open(QIODevice::WriteOnly), "collision fixture creation")) return 1;
    collision.close();
    const QString secondPath = PdfExporter::outputPath(temporaryDirectory.path(), "A title", timestamp);
    ...
    if (!require(!result.success && result.error.contains("user message"), "user message required"))
        return 1;
    qDeleteAll(messages);
    messages.clear();
    ...
    if (!require(htmlError.isEmpty() && html.contains("Rendered Markdown")
                 && !html.contains("API-SECRET-SYSTEM-PROMPT"), "HTML source secret exclusion"))
        return 1;
    ...
    if (!require(pdf.open(QIODevice::ReadOnly) && pdf.size() > 0 && pdf.read(4) == "%PDF",
                 "valid PDF output")) return 1;
    pdf.close();

    const PdfExporter::Result repeated = PdfExporter::exportMessages(messages, temporaryDirectory.path());
    if (!require(repeated.success && repeated.path != result.path, "repeated export has distinct path"))
        return 1;

    const QString fileDestination = temporaryDirectory.path() + "/not-a-directory";
    QFile destination(fileDestination);
    if (!require(destination.open(QIODevice::WriteOnly), "non-directory fixture creation")) return 1;
    destination.close();
    const PdfExporter::Result invalidDestination = PdfExporter::exportMessages(messages, fileDestination);
    if (!require(!invalidDestination.success && invalidDestination.error.contains("not a directory"),
                 "non-directory destination failure")) return 1;
