...
int main(int argc, char *argv[])
{
...

    {
        ...
        if (!require(tempDir.isValid(), "tempDir valid")) return 1;
        ...
        if (!require(store.saveSession(session), "save session") ||
            ...
            !require(session.title == "Hello", "title derivation")) return 1;
        ...
        if (!require(loaded.size() == 1, "loaded size 1") ||
            ...
            !require(loaded[0].messages[0]->content() == "Hello", "loaded message content")) return 1;
...

    {
        ...
        if (!require(!store.saveSession(session), "skip empty save") ||
            !require(store.loadAllSessions().size() == 0, "store empty after skip")) return 1;
    }

    {
        ...
        Session session = SessionStore::createSession(messages);
        if (!require(!store.saveSession(session), "skip no-user-message save")) return 1;
...

    {
        ...
        if (!require(loaded.size() == 3, "loaded size 3") ||
            ...
            !require(loaded[2].title == "Oldest", "oldest last")) return 1;
        clearSessions(loaded);
    }

    {
        ...
        if (!require(loaded.size() == 1, "skip corrupt file") ||
            !require(loaded[0].title == "Valid", "valid session preserved")) return 1;
        clearSessions(loaded);
    }

    {
        ...
        if (!require(store.deleteSession(id), "delete session") ||
            !require(store.loadAllSessions().size() == 0, "empty after delete")) return 1;
    }

    {
        ...
        if (!require(!store.saveSession(session), "reject traversal save") ||
            ...
            !require(!SessionStore::isValidId("../outside"), "traversal id invalid")) return 1;
        SessionStore::clearSession(session);
    }

    {
        ...
        QJsonObject badTimestamp = validObject(ids[1]); badTimestamp["timestamp"] = "not-a-date";
        QJsonObject badTitle = validObject(ids[2]); badTitle["title"] = 12;
        QJsonObject badMessages = validObject(ids[3]); badMessages["messages"] = "not-an-array";
        QJsonObject badRole = validObject(ids[4]);
        QJsonArray roleMessages = badRole["messages"].toArray();
        QJsonObject systemMessage = roleMessages[0].toObject(); systemMessage["role"] = static_cast<int>(ChatMessage::System);
        roleMessages[0] = systemMessage; badRole["messages"] = roleMessages;
        QJsonObject badContent = validObject(ids[5]);
        QJsonArray contentMessages = badContent["messages"].toArray();
        QJsonObject numericContent = contentMessages[0].toObject(); numericContent["content"] = 42;
        contentMessages[0] = numericContent; badContent["messages"] = contentMessages;
        const QJsonObject objects[] = { mismatch, badTimestamp, badTitle, badMessages, badRole, badContent };
        for (int i = 0; i < 6; ++i)
            if (!require(writeJson(tempDir.path() + "/" + ids[i] + ".json", objects[i]), "write malformed fixture")) return 1;
        if (!require(store.loadAllSessions().isEmpty(), "skip all structurally invalid sessions")) return 1;
    }

    {
        ...
        if (!require(store.saveSession(session), "save filtered session") ||
            !require(session.title == "First user title", "normalize title whitespace")) return 1;
        QFile file(tempDir.path() + "/" + session.id + ".json");
        if (!require(file.open(QIODevice::ReadOnly), "open saved json")) return 1;
        ...
        if (!require(!json.contains("SECRET SYSTEM PROMPT"), "exclude system content") ||
            ...
            !require(!json.contains("systemPrompt"), "exclude system prompt field")) return 1;
        SessionStore::clearSession(session);
    }

    {
        ...
        if (!require(store.saveSession(older), "save older")) return 1;
        const QString selectedId = older.id;
        QThread::msleep(1100);
        Session newer;
        newer.messages.append(new ChatMessage(ChatMessage::User, "Newer"));
        if (!require(store.saveSession(newer), "save newer")) return 1;
        SessionListModel model(&store);
        QList<ChatMessage*> selected = model.messagesById(selectedId);
        if (!require(selected.size() == 1 && selected[0]->content() == "Selected older", "stable lookup after reorder")) return 1;
        qDeleteAll(selected);
        model.deleteSessionById(selectedId);
        if (!require(model.rowCount() == 1, "stable delete removes one") ||
            !require(model.sessionId(0) == newer.id, "stable delete keeps newer")) return 1;
